# wove cloud (hosted edition) — design
...
## What cloud sells
1. Zero-ops hosting: one click → a running wove site with backups, TLS, custom domain.
...
- Control plane (`apps/cloud/api`, also Bun+Hono+Drizzle on Postgres): orgs, users, sites, domains,
  billing (Stripe), provisioning (create volume + machine, set `WOVE_*` env), backups (Litestream → S3).
- Router/edge: wildcard `*.wove.app` + custom domains → site machine. Cache public GETs at edge.
...
(`WOVE_DB`, `WOVE_MEDIA_DIR`/S3 vars, `WOVE_SITE_URL`), and an `admin bootstrap token`
