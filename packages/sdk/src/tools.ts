...
export const ToolCatalog = {
  ...
  // collections
  "collection.list":   { input: z.object({}), output: z.array(S.Collection.extend({ entryCount: z.number().int(), entrySchema: z.unknown().describe("JSON schema for this collection's entry data") })), scopes: ["content:read"] },
  "collection.get":    { input: z.object({ slug: S.Slug }), output: S.Collection.extend({ entryCount: z.number().int(), entrySchema: z.unknown() }), scopes: ["content:read"] },
  "collection.create": { input: z.object({ slug: S.Slug.optional(), name: z.string().min(1), namePlural: z.string().optional(), icon: z.string().optional(), fields: z.array(S.CollectionField).min(1), titleFieldKey: z.string().optional().describe("defaults to the first text field"), public: z.boolean().optional() }), output: S.Collection, scopes: ["settings:write"] },
  "collection.update": { input: z.object({ slug: S.Slug, name: z.string().optional(), namePlural: z.string().optional(), icon: z.string().optional(), fields: z.array(S.CollectionField).min(1).optional(), titleFieldKey: z.string().optional(), public: z.boolean().optional() }), output: S.Collection, scopes: ["settings:write"] },
  "collection.delete": { input: z.object({ slug: S.Slug, deleteEntries: z.boolean().default(false) }), output: z.object({ ok: z.literal(true), deletedEntries: z.number().int() }), scopes: ["settings:write"] },
  "entry.list":   { input: z.object({ collection: S.Slug, status: z.enum(["draft", "published"]).optional(), q: z.string().optional(), limit: z.number().int().min(1).max(100).default(20), cursor: z.string().optional() }), output: S.Page(S.CollectionEntry), scopes: ["content:read"] },
  "entry.get":    { input: z.object({ collection: S.Slug, id: S.Id }), output: S.CollectionEntry, scopes: ["content:read"] },
  "entry.create": { input: z.object({ collection: S.Slug, data: z.record(z.unknown()), status: z.enum(["draft", "published"]).default("draft") }), output: S.CollectionEntry, scopes: ["content:write"] },
  "entry.update": { input: z.object({ collection: S.Slug, id: S.Id, data: z.record(z.unknown()).optional().describe("merged by key; set a key to null to clear it"), status: z.enum(["draft", "published"]).optional() }), output: S.CollectionEntry, scopes: ["content:write"] },
  "entry.delete": { input: z.object({ collection: S.Slug, id: S.Id }), output: z.object({ ok: z.literal(true) }), scopes: ["content:write"] },
  // menus
...
export const ToolDescriptions: Record<ToolName, string> = {
  ...
  "collection.list": "List collections (custom content types) with entry counts and the JSON schema of their entry data.",
  "collection.get": "Fetch one collection definition with its entry JSON schema.",
  "collection.create": "Create a collection: name + typed fields (text, markdown, number, date, select, image…). Entries validate against them.",
  "collection.update": "Update a collection's name/fields/visibility. Removing a field keeps old values in existing entries (ignored).",
  "collection.delete": "Delete a collection; refuses when it has entries unless deleteEntries is true.",
  "entry.list": "List a collection's entries with optional status/search filters.",
  "entry.get": "Fetch one entry.",
  "entry.create": "Create an entry; `data` is validated against the collection's fields.",
  "entry.update": "Update an entry; `data` merges by key (null clears a key).",
